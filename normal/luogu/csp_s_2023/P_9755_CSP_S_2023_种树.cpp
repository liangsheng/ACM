...
__int128 cal(int i, int x, int t) {
    if (c[i] >= 0) {
        return (__int128) b[i] * (t - x + 1) + (__int128) c[i] * (x + t) * (t - x + 1) / 2;
    }
    if (x > p[i]) return t - x + 1;
    if (t <= p[i]) return (__int128) b[i] * (t - x + 1) + (__int128) c[i] * (x + t) * (t - x + 1) / 2;
    return (__int128) b[i] * (p[i] - x + 1) + (__int128) c[i] * (x + p[i]) * (p[i] - x + 1) / 2 + t - p[i];
}
...
int gao(int i, int t) {
    ...
    if (tol < a[i]) return -1;
    int l = 1, r = t, mid;
    // for (int j = t; j >= 1; j--) if (cal(i, j, t) >= a[i]) {
    //     l = j;
    //     break;
    // }
    // if (l == -1) return -1;
    ...
    // for (int j = t; j >= 1; j--) if (cal(i, j, t) >= a[i]) return j;
    // return -1;
}
