...
int main(int argc, char** argv) {
    ...
    for (int t = 0; t < T; t++) {
        s[t] = rnd.next(n[t]) + 1;
        e[t] = rnd.next(n[t]) + 1;
        while (e[t] == s[t]) e[t] = rnd.next(n[t]) + 1;
        if (s[t] > e[t]) swap(s[t], e[t]);
        k[t] = rnd.next(k[t] / 2) + k[t] / 2;
        for (int i = 1; i <= n[t]; i++) {
            int j = min(n[t], i + k[t] - 1);
            for (int p = i; p < j; p++) {
                g[t].push_back({p, p + 1, (rnd.next(4) <= 2 ? 1 : -1) * rnd.next(a[t])});
            }
            g[t].push_back({j, i, (rnd.next(4) <= 2 ? 1 : -1) * rnd.next(a[t])});
            int u = min(i + k[t], n[t]), v = min(u + k[t] - 1, n[t]), d = v - u + 1;
            if (d > 0) {
                for (int p = i; p <= j; p++) {
                    g[t].push_back({p, rnd.next(d) + u, (rnd.next(4) <= 2 ? 1 : -1) * rnd.next(a[t])});
                }
            }
            i = j;
        }
