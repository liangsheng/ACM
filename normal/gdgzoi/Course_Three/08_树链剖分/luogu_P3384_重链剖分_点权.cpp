 ...
 * 题意: 给你一棵带点权树, 可以修改点权, 询问 2 点之间的路劲或子树的权值和
 */
...
void push_down(int k, int s, int t) {
    ...
    a[l] = (a[l] + sl * b[k] % MOD) % MOD;
    a[r] = (a[r] + sr * b[k] % MOD) % MOD;
    b[l] += b[k], b[r] += b[k];
    b[k] = 0;
}
...
int query(int k, int s, int t, int l, int r) {
    ...
    ans = (ans + query(k << 1 | 1, mid + 1, t, l, r)) % MOD;
    return ans;
}
/*-------------------------- 线段树 end ------------------*/
...
int ask_path(int u, int v) {
    ...
    return (ans + query(1, 1, n, dfn[u], dfn[v])) % MOD;
}
...
int main() {
    ...
    while (cin >> n >> q >> r >> MOD) {
        ...
        while (q--) {
            cin >> cmd;
            if (cmd == 1) {         // 表示将树从 x 到 y 结点最短路径上所有节点的值都加上 z
                cin >> x >> y >> z;
                update_path(x, y, z);
            } else if (cmd == 2) {  // 表示求树从 x 到 y 结点最短路径上所有节点的值之和
                cin >> x >> y;
                cout << ask_path(x, y) << '\n';
            } else if (cmd == 3) {  // 表示将以 x 为根节点的子树内所有节点值都加上 z
                cin >> x >> z;
                update(1, 1, n, dfn[x], dfn[x] + sz[x] - 1, z);
            } else if (cmd == 4) {  // 表示求以 x 为根节点的子树内所有节点值之和
                cin >> x;
