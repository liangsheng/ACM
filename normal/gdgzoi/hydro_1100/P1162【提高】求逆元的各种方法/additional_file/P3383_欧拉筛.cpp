...
// oiwiki: https://oiwiki.org/math/number-theory/sieve/
// 欧拉筛: https://www.bilibili.com/video/BV1H8411E7hn/?share_source=copy_web&vd_source=037e882c0cdcb03fb30079bba610dae3
// 欧拉函数: https://oiwiki.org/math/number-theory/euler-totient/
// 莫比乌斯函数: https://oiwiki.org/math/number-theory/mobius/
struct EulerSievePhi {
    int n, m;
    vector<int> mp, p;    // 欧拉筛相关
    vector<int> phi, mu;  // phi: 欧拉函数, mu: 莫比乌斯函数

    // 假设 i = p1^a1 * p2^a2, (p1 < p2)
    // phi[i] = i * (1 - 1/p1) * (1 - 1/p2)
    EulerSievePhi(int n_) {
        n = n_;
        mp.assign(n + 1, 0);   // mp[i] 表示 i 的最小质因子
        phi.assign(n + 1, 0);
        mu.assign(n + 1, 0);
...
struct EulerSieve {
    ...
    EulerSieve(int n_) {
        ...
        for (int i = 2; i <= n; i++) {
            if (!mp[i]) p.push_back(i), m++, mp[i] = i;
            for (int j = 0; j < m && i * p[j] <= n; j++) {
                mp[i * p[j]] = p[j];
                if (i % p[j] == 0) break;
            }
...
// oiwiki: https://oiwiki.org/math/number-theory/sieve/
// 欧拉筛: https://www.bilibili.com/video/BV1H8411E7hn/?share_source=copy_web&vd_source=037e882c0cdcb03fb30079bba610dae3
// bitset: https://oi-wiki.org/lang/csl/bitset/
const static int n = 10000000;
    
vector<int> p;
bitset<n> b;

void eulerSieveSimple() {
    for (int i = 2; i <= n; i++) {
...
int eratosthenes(int n) {
    ...
    for(int i = 2; (i64) i * i <= n; i++) if (is_p[i]) {
...
int main() {
    ...
    // EulerSieve e(1000000);        // 只有欧拉筛
    // EulerSieveFactor e(1000000);  // 欧拉筛, 计算约数个数, 约数和
    // EulerSievePhi e(1000000);     // 欧拉筛, 计算欧拉函数, 莫比乌斯函数

    eulerSieveSimple();              // bitset 优化的欧拉筛
    int n, m, k;
    cin >> n >> m;
    for (int i = 0; i < m; i++) {
        cin >> k;
