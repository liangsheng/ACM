...
N = [4, 5, 10, 20, 50, 100, 500, 1000, 10000, 10000]
M = [5, 2, 5, 5, 5, 10, 10, 20, 20, 50]
T = len(N) - 1
...
for t in range(1, T + 1):
    ...
    n = randint(N[t - 1], N[t])
    q = min(randint(n - 1, 10 * n), 200000)
    io.input_writeln(n, q)
    for _ in range(q):
        c, u, v = randint(1, 2), randint(1, n), randint(1, n)
        if c == 2:
            v = randint(1, 10)
        io.input_writeln(c, u, v)
