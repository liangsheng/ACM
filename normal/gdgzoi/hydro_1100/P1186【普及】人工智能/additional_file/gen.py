...
N = [1, 1, 1, 1, 1, 10, 20, 50, 100, 200, 500, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000]
...
for t in range(1, T + 1):
    ...
    if t >= 5 and t <= 8:
        k = 1
    s = [chr(randint(ord('r') - 5, ord('r'))) for _ in range(k * n)]       
    s = ''.join(s)
    m = len(s)
    a = [i for i in range(1, m)]
    b = random.sample(a, n - 1)
    b.sort()
    if t >= 9 and t <= 12:
        b[0] = 1
    if t >= 13 and t <= 16:
    ...
    io.input_writeln(s)
    io.input_writeln(n)
    io.input_writeln(' '.join(map(str, b)))    
