...
D = [1, 1, 1, 2, 2, 5, 10, 20, 50, 100, 100]
N = [1, 1, 5, 1, 5, 5, 10, 20, 50, 100, 100]
M = [1, 10, 10, 20, 20, 50, 100, 200, 500, 1000, 1000]
L = [1, 10, 20, 30, 40, 50, 100, 200, 500, 600, 1000]
T = len(N) - 1
...
for t in range(1, T + 1):
    ...
    d, n, m = D[t], N[t], M[t];
    io.input_writeln(d, n, m)
    for _ in range(d):
            a = [randint(max(1, m // 5 - 5), m // 5) for i in range(n)]
            io.input_writeln(' '.join(map(str, a)))
