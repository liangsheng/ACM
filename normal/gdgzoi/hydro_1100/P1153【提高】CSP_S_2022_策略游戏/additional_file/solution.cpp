    ...
    while (q--) {
        ...
        if (min_b.query(l2, r2) >= 0) {                                          // b[l2..r2] >= 0
            if (max_a.query(l1, r1) <= 0) {
                cout << (LL) max_a.query(l1, r1) * max_b.query(l2, r2) << '\n';  // 负
            } else {
                cout << (LL) max_a.query(l1, r1) * min_b.query(l2, r2) << '\n';  // 正
            }
        } else if (max_b.query(l2, r2) <= 0) {                                   // b[l2..r2] <= 0
            if (min_a.query(l1, r1) >= 0) {
                cout << (LL) min_a.query(l1, r1) * min_b.query(l2, r2) << '\n';  // 负
            } else {
                cout << (LL) min_a.query(l1, r1) * max_b.query(l2, r2) << '\n';  // 正
            }
        } else {
            if (min_a.query(l1, r1) >= 0) {
                cout << (LL) min_a.query(l1, r1) * min_b.query(l2, r2) << '\n';  // 负
                ...
                LL ans = (LL) min_a_g0.query(l1, r1) * min_b.query(l2, r2);
