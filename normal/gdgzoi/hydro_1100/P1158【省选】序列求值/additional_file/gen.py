...
N = [4, 5, 10, 15, 20, 50, 100000, 1000000, 100000000, 1000000000, 10000000000]
...
for i in range(1, T + 1):
    ...
    n = randint(N[i - 1], N[i])
    f1, f2 = randint(M[i - 1], M[i]), randint(M[i - 1], M[i])
    f3, c = randint(M[i - 1], M[i]), randint(M[i - 1], M[i])
    io.input_writeln(n, f1, f2, f3, c)
    # n, q = N[i], N[i]
